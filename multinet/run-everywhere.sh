...
usage() {
  ...
  echo "Usage: ${0} [-nsv] [-fFILE] COMMAND" >&2
  echo 'Executes COMMAND as a single command on every server.' >&2I
  echo "  -f FILE   Use FILE for the list of servers. Default ${SERVER_LIST}" >&2
  echo "  -n        Dry run mode. Display the COMMAND that would have been executed and exit." >&2
  echo "  -s        Execute the COMMAND using sudo on the remote server." >&2
  echo '  -v        Verbose mode. Displays the server name before executing COMMANDD.' >&2
  exit 1
}
